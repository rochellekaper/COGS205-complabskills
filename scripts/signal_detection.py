...
class SignalDetection:
    def __init__(self, hits, misses, false_alarms, correct_rejections):
        ...
        #avoid dividing by zero, and taking ppf(0)
        if self.hits == 0 and self.misses == 0:
            raise ValueError("both hits and misses cannot be 0")   
        
        if self.false_alarms == 0 and self.correct_rejections == 0:
            raise ValueError("both false alarms and correct rejections cannot be 0")  
    ...
    def plot_sdt(self):
        ...
        x = np.linspace(mu_noise - 4, self.d_prime() + 4, 1000)
        peak = norm.pdf(0, 0, sigma)
        plt.plot(x, norm.pdf(x, self.d_prime(), sigma), label = 'Signal', color = 'blue')
        plt.plot(x, norm.pdf(x, mu_noise, sigma), label = 'Signal + Noise', color = 'red')
        ax.set_title('Signal to Noise')
        ax.annotate("d'", xy=(mu_noise, peak), xytext=(self.d_prime(), peak), 
